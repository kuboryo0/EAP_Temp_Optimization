...
void heappush(MinHeap* heap, Node* value) {
    ...
    int i = heap->size - 1;
    int parent = (i - 1) / 2;
    while (i > 0 && heap->data[parent].priority > heap->data[i].priority) {
...
void astar(Allocation allocation, TemporaryRoads temps, double soil_amount[GRID_SIZE_X][GRID_SIZE_Y],
           int* used_temp_list, double *total_cost, Path *path) {
    ...
    heappush(&open_set, &(Node){.priority = 0, .coord = start});
    ...
    while (open_set.size > 0) {
        Node current_node = heappop(&open_set);
        ...
        for (int i = 0; i < 8; i++) {
            ...
            Coord neighbor = (Coord){current.x + dx, current.y + dy};
            
            // グリッド範囲内の座標か確認
            if (neighbor.x < 0 || neighbor.x >= GRID_SIZE_X || neighbor.y < 0 || neighbor.y >= GRID_SIZE_Y) {
                continue;
            }
            ...
            if (new_cost < cost_so_far[neighbor.x][neighbor.y]) {
                ...
                heappush(&open_set, &(Node){.priority = priority, .coord = neighbor});
    ...
    while (current.x != start.x || current.y != start.y) {
        path->coord[path_length++] = current;
        Coord next = came_from[current.x][current.y];
        
        // 仮設道路を使ったかどうかをused_temp_listに記録
        CoordPair search_pair;
        if (current.x < next.x || (current.x == next.x && current.y < next.y)) {
            search_pair.coords[0] = current;
            search_pair.coords[1] = next;
        } else {
            search_pair.coords[0] = next;
            search_pair.coords[1] = current;
        }

        // 仮設道路のリストからそのペアが存在するかを検索
        qsort(temps.coordpair, temps.count, sizeof(CoordPair), compare_search);
        CoordPair *found = bsearch(&search_pair, temps.coordpair, temps.count, sizeof(CoordPair), compare_search);

        if (found != NULL) {
            // 仮設道路を利用した場合、used_temp_listを更新
            int index = found - temps.coordpair;
    ...
    path->size = path_length + 1;
    path->coord[path_length++] = start;
    path->coord = resize_path(path->coord, path_length);
    
    // 結果表示
    printf("Path: \n");
    for (int i = path_length - 1; i >= 0; i--) {
        printf("(%d, %d)\n", path->coord[i].x, path->coord[i].y);
    }
...
int process_allocations(
    ...
    double built_length_list[],
    int used_temp_list[][solution.count],
    Path path[],
    double* total_cost
) {
    //仮設道路の初期化
    ...
    for (int i = 0; i < step; i++) {
        ...
        //built_length_listを出力
        printf("after temp_in_step\n");
        for (int j = 0; j < temps.count; j++) {
            printf("temps.coordpair[%d]: (%d,%d),(%d,%d)\n", j, temps.coordpair[j].coords[0].x, temps.coordpair[j].coords[0].y, temps.coordpair[j].coords[1].x, temps.coordpair[j].coords[1].y);
            printf("temps.statuslist[%d]: (%d,%d)\n", j, temps.statuslist[j].status[0], temps.statuslist[j].status[1]);
        }
        //仮設道路を出力
        for (int j = 0; j < temps.count; j++) {
            printf("temps.coordpair[%d]: (%d,%d),(%d,%d)\n", j, temps.coordpair[j].coords[0].x, temps.coordpair[j].coords[0].y, temps.coordpair[j].coords[1].x, temps.coordpair[j].coords[1].y);
        ...
        for (int j = 0; j < path[i].size; j++) {
            printf("path[%d]: (%d,%d)\n", j, path[i].coord[j].x, path[i].coord[j].y);
        }
        //costを出力
        printf("cost: %f\n", cost);
        //used_temp_listを出力
        printf("used_temp_list[%d]: %d\n", i, used_temp_list[i]);
...
int main() {
    ...
    solution.timings[0] = (int[]){1, 0, 1};
    solution.timings[1] = (int[]){0, 1, 1};
    solution.timings[2] = (int[]){0, 1, 0};
    ...
    Path path[solution.step];
    for(int i = 0; i < solution.step; i++){
        path[i].size = GRID_SIZE_X * GRID_SIZE_Y;
        path[i].coord = (Coord *)malloc(path[i].size * sizeof(Coord));
        if (path[i].coord == NULL) {
    ...
    //     printf("path[%d]: (%d,%d)\n", i, path.coord[i].x, path.coord[i].y);
    // }
    // //used_temp_listを出力
    // for (int i = 0; i < solution.count; i++) {
    //     for (int j = 0; j < solution.step; j++) {
    //         printf("used_temp_list[%d][%d]: %d\n", i, j, used_temp_list[i][j]);
    //     }
    // }

    //process_allocationsを実行
    double total_cost = 0;
    double built_length_list[solution.count];
    for (int i = 0; i < solution.count; i++) {
        built_length_list[i] = 0;
    }
