...
Solution generateSolutionTiming(
    ...
    double soil_amount[GRID_SIZE_X][GRID_SIZE_Y]
        ...
        // std::cout << "used_road_flow" << std::endl;
...
int main() {
   ...
   normalize_pair(coordpair8);
    ...
    // std::cout << "初期ネットワーク:" << std::endl;
    // networkList.printConverted();

    // // ノード1と2の接続を削除（エッジを削除）
    // std::cout << "\nノード1と2の接続を削除:" << std::endl;
    // networkList.connectNetworkList[0].edgeList[1][2] = 0; // エッジ(1,2)削除
    // networkList.connectNetworkList[0].edgeList[2][1] = 0; // 対称性維持のため明示的に削除

    // // 更新後のネットワークを取得
    // connectNetworkList updatedNetworkList = updateConnectNetwork(networkList, 0);

    // std::cout << "更新後のネットワーク:" << std::endl;
    // updatedNetworkList.printConverted();
