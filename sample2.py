    ...
    def shortest_route_search(cut_indicies, fill_indices,temp_list_cleaned):
        ...
        for i in range(len(cut_indices)):
            for j in range(len(fill_indices)):
                temp_list_cleaned.insert(0,[cut_indices[i][0]])
                temp_list_cleaned.append([fill_indices[j][0]])
                # print("temp_list_cleaned after",temp_list_cleaned)
                cost[i][j] = Warshall_Floyd(temp_list_cleaned)
        return cost
    # print("temp_list_cleaned after",temp_list_cleaned)


    cost = shortest_route_search(cut_indices, fill_indices,temp_list_cleaned)
...
for i in range(len(cut_indices)):
    cut_indices_float[i][0][0] = float(cut_indices[i][0][0])
    cut_indices_float[i][0][1] = float(cut_indices[i][0][1])
#盛土の座標と土量
fill_indices = [[(0, 0),1], [(0, 1),1], [(0, 2),1], [(1, 1),2],[(3, 2),2]]
fill_indices_float = fill_indices
for i in range(len(fill_indices)):
    fill_indices_float[i][0][0] = float(fill_indices[i][0][0])
    fill_indices_float[i][0][1] = float(fill_indices[i][0][1])
#仮設道路の情報
temp = [[(0, 0), (0, 1)],[(1, 1),(2, 2)],[(1, 2),(2, 1)]]
...
# コスト行列の作成
costs = calculate_cost(cut_indices_float, fill_indices_float,temp)
